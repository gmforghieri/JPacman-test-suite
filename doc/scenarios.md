...
```
As a player,
 I want to play another level after having won the previous level;
So that I can continue playing pacman.

Scenario S5.1: Winning the game.
Given the player is playing the game;
When  the player wins the game aka consumes the last pellet;
Then  the "Start" button will change its functionality to starting the next level.

Scenario S5.2: The game is over.
Given the player has won 3 times in a row and is playing the 4th time;
When  the player consumed the last pellet of the 4th game;
Then no more level will be started, the game is over.
```
